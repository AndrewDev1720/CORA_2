...
class NodeExplainerEdgeMulti(torch.nn.Module):
    def __init__(self, base_model, G_dataset, test_indices, args, fix_exp=None):
        ...
        self.base_model = base_model
        self.base_model.eval()
        self.G_dataset = G_dataset  # Should be of type torch_geometric.data.Data
        self.test_indices = test_indices
        self.args = args
        if fix_exp:
            self.fix_exp = fix_exp * 2
    ...
    def explain_nodes_gnn_stats(self, listNodes):
        exp_dict = {}  # {'node_id': masked_adj}
        num_dict = {}  # {'node_id': num_explanations}
        pred_label_dict = {}
        t_gid = []
        ...
        for gid in tqdm.tqdm(listNodes[:num_nodes_to_explain]):
            ...
            # ori_label = self.G_dataset.y[gid].item()


            # Proceed only if the prediction is correct
            # if ori_pred_label == ori_label:
            t_gid.append(gid.item())
            masked_adj, exp_num = self.explain(gid, ori_pred_label)
            exp_dict[gid.item()] = masked_adj
            num_dict[gid.item()] = exp_num
            pred_label_dict[gid.item()] = ori_pred_label

        print('Average number of explanations:', sum(num_dict.values()) / len(num_dict.keys()))
        return exp_dict, num_dict

    def explain(self, node_idx, pred_label):
        try:
            ...
            return masked_adj.cpu().detach(), exp_num

        except Exception as e:
            print(f"Error during explanation for node {node_idx}: {e}")
            return None, 0
...
class ExplainModelNodeMulti(torch.nn.Module):
    ...
    def loss(self, pred1, pred2, pred_label, gam, lam, alp):
        relu = torch.nn.ReLU()
        pred1_prob = F.softmax(pred1, dim=0)
        pred2_prob = F.softmax(pred2, dim=0)

        num_classes = pred1_prob.size(0)
        mask = torch.ones(num_classes, dtype=torch.bool, device=self.device)
        mask[pred_label] = False

        f_next = pred1_prob[mask].max()
        cf_next = pred2_prob[mask].max()
